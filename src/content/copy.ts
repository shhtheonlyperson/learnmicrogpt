...
export function getCopy(locale: Locale) {
  ...
  const ui = isZh
    ? {
        ...
        sectionDescriptions: {
          loop: '照 gist 真正執行的順序，從文字檔一路走到模型亂數噴出來的中文名字。每一步的原始碼 context 都直接 inline 在動畫裡。',
          atlas: '這份 gist 短到可以逐段畫成地圖。點選不同行範圍，下方 detail 卡就會直接展開對應區段。',
    ...
    : {
        ...
        sectionDescriptions: {
          loop: 'Follow the gist in the order it actually runs, from a text file to the model blurting out new names. Each step now carries its source context inline inside the animation.',
          atlas: 'This gist is short enough to map section by section. Click a line range and the detail card expands the matching chunk inline.',
        ...
        labels: {
          line: 'lines {lineRange}', openSource: 'open source', openGistRange: 'open gist range', liveSourcePanel: 'Live source panel', sharedSourceHint: 'The matching snippet now expands inline instead of being pushed into a separate side panel.', rightPanelHint: 'The fixed source panel on the right stays in sync with the loop step you are looking at.', primitivesEyebrow: 'This part deserves blunt language', primitivesTitle: 'It is a toy because it is small, not because it is fake', primitivesBody: 'Each primitive below keeps one core GPT idea intact while removing the industrial machinery that usually hogs the spotlight. That is why `microgpt.py` teaches so well: you get the algorithm itself, not the entourage.', primitiveTag: 'Primitive', kept: 'kept', stripped: 'stripped', spine: 'the algorithmic spine', machinery: 'scaling machinery', quickSlice: 'quick reference slice', lossTitle: 'Loss drops noisily. No miracles. Just training.', lossBody: `In a quick ${proofArtifacts.numSteps}-step run, the script already starts producing shapes that look suspiciously like names. The chart below uses the real numbers from that sprint, not presentation-friendly fake drama.`, readProof: 'how to read this evidence', proofTitle: 'The lack of perfection is the point.', loopEyebrow: 'Core algorithm, unfolded in order', loopBody: 'The loop plays straight through, and each step keeps its source context inline on the stage. The point is not to show off. The point is to unfold a real GPT training loop in the order it actually executes.', sourcePanelOriginLoop: 'Loop step', sourcePanelOriginAtlas: 'Atlas section', sourcePanelEyebrowAtlas: 'Single-file atlas', start: 'start', executionTrace: 'execution trace', across: `across ${11} transitions`, atlasEyebrow: 'lines {lineRange}', handoff: 'handoff', to: 'to', currentStage: 'current stage', nextStage: 'next stage', liveTransitionLog: 'live transition log', currentSource: 'current source lines', stageStatus: { current: 'in progress', next: 'up next', idle: 'idle' },
        },
      }
